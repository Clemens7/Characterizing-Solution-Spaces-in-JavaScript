...
window.removeCartItem = (buttonNode) => {
  const numberOfItemsToRemove = 1;
  let cart = retrieveCart();

  const itemToRemove = buttonNode.parentNode.parentNode
  let itemId = parseInt(itemToRemove.id);

  if (cart !== null) {
    cart.splice(itemId, numberOfItemsToRemove);
