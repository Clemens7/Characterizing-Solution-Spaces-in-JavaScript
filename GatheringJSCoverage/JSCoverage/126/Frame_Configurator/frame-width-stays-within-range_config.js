...
function connectInputForms(idNumber, idRange) {
    ...
    if (input1 === null || !(input1 instanceof HTMLInputElement)) 
        ...
        input1.addEventListener("change", () => {
            // adjust an invalid input to either minimum, rounded, or maximum value (specified via given tests)
            if (input1.validity.rangeUnderflow)
                input1.value = input1.min;
            else if (input1.validity.rangeOverflow)
                input1.value = input1.max;
            else if (input1.validity.stepMismatch) {
                let newValue = parseFloat(input1.value); // e.g. 3.55
                newValue = Math.round(newValue * 10); // becomes 36
                input1.value = (newValue / 10.0).toString(); // then 3.6
            }
