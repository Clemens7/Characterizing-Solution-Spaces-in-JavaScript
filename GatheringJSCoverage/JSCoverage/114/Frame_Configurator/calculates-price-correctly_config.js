...
document.getElementById('frameWidth').addEventListener("focusout", function(){updateSliderF('frameWidth','frameWidthR')});
...
function updateSliderF(slideValue,output) {
	slideValue=document.getElementById(slideValue);
	var output = document.getElementById(output);
	if(slideValue.value>=2 && slideValue.value<=5){
...
function updatePricePreviewImg(){
	...
	if(price.toString().substring(price.toString().indexOf("."),price.toString().length).length ==2)price= price+"0";
