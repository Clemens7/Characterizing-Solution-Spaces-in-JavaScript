...
function getShippingCostsElement(){
	return document.getElementById("price-shipping");
}
...
function getTotalCostsElement(){
	return document.getElementById("price-total");
}
			...
			if (response.status !== 200) 
	  		...
	  		response.json().then(function(data) {  
		  		...
		  		for (let i = 0; i < data.destinations.length; i++) {
			  		if(data.destinations[i].country === getSelectCountry().value){
			  			value = data.destinations[i].cost;
			  			getPayButton().disabled = false;
			  			getShippingCostsElement().innerHTML = (value / 100).toFixed(2);
			  			getTotalCostsElement().innerHTML = (calculateSubTotalPrice() + (value / 100)).toFixed(2);
			  			return;
			  		}
    			}});
    	}  
