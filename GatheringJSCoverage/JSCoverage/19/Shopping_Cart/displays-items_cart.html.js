      ...
      async function displayCartItems(itemsInCart) {
          ...
          for (let i = itemsInCart.length - 1; i >= 0; i--) {
              ...
              const desciption = `${(itemsInCart[i].printSize == "S") ? "Small" : (itemsInCart[i].printSize == "M") ? "Medium" } print in a ${parseFloat(itemsInCart[i].frameWidth/10)} cm ${itemsInCart[i].frameStyle} ${(itemsInCart[i].matWidth == 0)  : `frame with a ${parseFloat(itemsInCart[i].matWidth/10)} cm ${itemsInCart[i].matColor} mat.`}`;
