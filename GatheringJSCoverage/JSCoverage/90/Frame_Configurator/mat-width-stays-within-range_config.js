...
function setUpListeners() {
    ...
    matWidthNumberInput.addEventListener('change', (evt) => {
...
function getCorrectValue(value, min, max) {
    if (value < min) return min;
    if (value > max) return max;

    let ret = value.toFixed(1);
    return parseFloat(ret) % 1 === 0  : ret;
}
...
function evtValueChecks(evt) {
    ...
    if (!isValueCorrect(src.valueAsNumber, src.min, src.max)) {
        value = getCorrectValue(src.valueAsNumber, src.min, src.max);
        src.value = value;
    };
    return value;
}
