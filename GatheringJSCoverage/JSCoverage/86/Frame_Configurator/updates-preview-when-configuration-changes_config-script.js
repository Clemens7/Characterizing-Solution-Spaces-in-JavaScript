...
for (let i=0; i<sizeSelector.length; i++) {
  sizeSelector[i].addEventListener('input', event => {
    printSize = event.target.value;
  });
}

for (let i=0; i<frameSelector.length; i++) {
  frameSelector[i].addEventListener('input', event => {
    frameStyle = event.target.value;
  });
}

for (let i=0; i<matSelector.length; i++) {
  matSelector[i].addEventListener('input', event => {
    matColor = event.target.value;
  });
}

numberBox[0].addEventListener('input', event => {
  slider[0].value = numberBox[0].value;
  constrainFrameWidth(numberBox[0].value);
});
...
function constrainFrameWidth(newFrameWidth) {
  frameWidth = Math.min(Math.max(newFrameWidth, 2), 5);
}
