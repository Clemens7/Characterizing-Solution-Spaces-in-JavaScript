        ...
        function setDefaultValues(params) {
            ...
            if (printSize) {
                frame_functions.setPrintSize(printSize);
                item.printSize = printSize;
            }

            if (frameStyle) {
                frame_functions.setFrameStyle(frameStyle);
                item.frameStyle = frameStyle;
            }

            if (matColor) {
                frame_functions.setMatColor(matColor);
                item.matColor = matColor;
            }

            if (frameWidth) {
                frame_functions.setFrameWidth(frameWidth / 10);
                item.frameWidth = frameWidth * 10;
            }

            if (matWidth) {
                frame_functions.setMatWidth(matWidth / 10);
                item.matWidth = matWidth * 10;
            }
