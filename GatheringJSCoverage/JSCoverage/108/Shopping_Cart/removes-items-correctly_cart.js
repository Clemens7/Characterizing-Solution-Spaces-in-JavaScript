...
export class Cart {
    ...
    static remove(arrayID) {
        console.log(arrayID);
        if (localStorage[this.key] && arrayID > -1) {
            let cart = JSON.parse(localStorage[this.key]);
            cart.splice(arrayID, 1);
...
export class CartContainer {
    ...
    async createCartItemElements(item, id) {
        ...
        cart_remove_item.onclick = function (event) {
            Cart.remove(cart_remove_item.id);
            cart_remove_item.parentElement.parentElement.remove();
            Cart.updateCartCurrentPrice();
            Cart.updateCartItemCounter();
        };
