...
async function loadCartElements() {
    ...
    for (let i = 0; i < deleteItemButton.length; i++) {
        ...
        button.addEventListener('click', function (event) {
            ...
            removeFromCart(event, cart[cart.length - (1 + i)], cart.length - (1 + i))
...
async function removeFromCart(event, { objectID, printSize, frameStyle, frameWidth, matWidth }, index) {
    let buttonClicked = event.target
    buttonClicked.parentElement.parentElement.remove()
    console.log('Deleting ' + objectID + ' from cart')

    if (cart.length === 1)  else {

        cart.splice(index, 1)
        await CacheService.set("cart", _ => cart)

        updatePriceTotal(printSize, frameStyle, frameWidth, matWidth)
        updateAmountCartItems()
    }
}

function updatePriceTotal(printSize, frameStyle, frameWidth, matWidth) {
...
function updateAmountCartItems() {
    cartLabelElement.innerHTML = `Cart ${cart ? `(` + cart.length + `)` }`
}
