...
frameWidth.addEventListener('change', event => {
    const value = formatFloat(validateInputRange(event.srcElement.value, 2, 5));
    image.frameWidth = value;
    frameWidth.value = value;
    frameWidthR.value = value;
    calculateTotalSize()
    calculateTotalPrice()
});
...
addCallBackToControls(printSizeControls, 'change', event => {
    image.printSize = event.srcElement.value;
    calculateTotalSize()
    calculateTotalPrice()
});
addCallBackToControls(frameStyleControls, 'change', event => {
    image.frameStyle = event.srcElement.value;
    calculateTotalPrice()
});
addCallBackToControls(matColorControls, 'change', event => {
    image.matColor = event.srcElement.value;
    calculateTotalPrice()
});
