...
if (frameWidth) frameWidth = +frameWidth / 10;
if (matWidth) matWidth = +matWidth / 10;
...
setStyle(frameStyle);
setColor(matColor);
if (frameWidth) frameWidthElement.value = frameWidth;
if (matWidth) matWidthElement.value = matWidth;
...
function setSize(size) {
    console.log(size);
    if (size != null) document.getElementById('print-size-' + size.toLocaleLowerCase()).checked = true;
    updateUI();
}
...
function setStyle(style) {
    if (style != null) document.getElementById('frame-style-' + style.toLocaleLowerCase()).checked = true;
    updateUI();
}
...
function setColor(color) {
    if (color != null) document.getElementById('mat-color-' + color.toLocaleLowerCase()).checked = true;
    updateUI();
}
