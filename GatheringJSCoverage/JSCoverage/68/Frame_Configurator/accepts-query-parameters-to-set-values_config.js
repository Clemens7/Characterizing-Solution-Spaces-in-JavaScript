...
function getFrame() {
    if (frame_style_classic.checked)  else if (frame_style_natural.checked)  else if (frame_style_shabby.checked) {
        return "shabby";
    }
...
function setParamFrameSize(paramFrameSize)
{
    switch(paramFrameSize)
    {
        
        case 'M':
        print_size_s.checked = false;
        print_size_m.checked = true;
        print_size_l.checked = false;
        break;
...
function setParamFrameStyle(paramFrameStyle)
{
    switch(paramFrameStyle)
    {
        
        
        case "shabby":
            frame_style_classic.checked = false;
            frame_style_natural.checked = false;
            frame_style_shabby.checked = true;
            frame_style_elegant.checked = false;
            break;
...
function setParamMatColor(paramMatColor)
{
    switch(paramMatColor)
    {
        
        case 'mint':
        mat_color_ivory.checked = false;
        mat_color_mint.checked = true;
        mat_color_wine.checked = false;
        mat_color_indigo.checked = false;
        mat_color_coal.checked = false;
        break;
...
function init() {
    ...
    if (urlParams.has('printSize')) {
       setParamFrameSize(urlParams.get('printSize'));
     }
     if(urlParams.has('frameStyle'))
     {
        setParamFrameStyle(urlParams.get('frameStyle'));
     }
     if(urlParams.has('frameWidth')){
        document.getElementById('frameWidth').value = urlParams.get('frameWidth')/10;
        frameRange.value = urlParams.get('frameWidth');
     }
    if(urlParams.has('matColor')){
        setParamMatColor(urlParams.get('matColor'));
    }
    if(urlParams.has('matWidth')){
    document.getElementById('matWidth').value = urlParams.get('matWidth')/10;
        matRange.value = urlParams.has('matWidth');
    }
