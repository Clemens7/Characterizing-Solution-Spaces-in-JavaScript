...
class Image {
    ...
    static generateHTMLCartItems (image,storageInfo) {
        ...
        removeBtn.onclick = () => {
            removeItem(storageInfo.objectID);
        };
...
function removeItem(objectID) {
    ...
    for (let cartDivID in cartContainer.children) {
        if (cartContainer.children[cartDivID].children[0].id === "preview-container-" + objectID) {

            const totalPriceTag = document.getElementById('price-total');
            const picturePrice = document.getElementById("price-" + objectID);

            totalPriceTag.innerText = (parseFloat(totalPriceTag.innerText) - parseFloat(picturePrice.innerText)).toFixed(2);
            cartContainer.children[cartDivID].remove();

            let storageCarts = JSON.parse(localStorage["cart"]);
            for (let storageCartID in storageCarts) {
                if (storageCarts[storageCartID].objectID === objectID) {
                    storageCarts.splice(storageCartID, 1);
                }
            }
            localStorage.setItem("cart", JSON.stringify(storageCarts));

            console.log(cartContainer.children.length);
