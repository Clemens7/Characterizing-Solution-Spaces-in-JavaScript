    ...
    function updateValue(target) {
      if (target.id.startsWith("print-size-"))else if (target.id.startsWith("frameWidth"))else if (target.id.startsWith("frame-style-"))else if (target.id.startsWith("matWidth")) {
    ...
    function setMatWidth(matWidth) {
      if (matWidth < 0){
        setBothMatWidths(0)
        console.log(`Wrong Frame Width: ${matWidth} (only between 0 and 10 cm)`)
        return;
      }
      if (matWidth > 10){
        setBothMatWidths(10)
