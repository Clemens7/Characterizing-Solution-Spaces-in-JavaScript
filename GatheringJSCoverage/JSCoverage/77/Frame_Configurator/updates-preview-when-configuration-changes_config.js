...
function setEventListenersForInputs(artwork) {
    ...
    for (let i = 0; i < printSizeRadioButtons.length; i++) {
        printSizeRadioButtons[i].addEventListener("change", () => {
            artwork.printSize = document.querySelector('input[name="printSize"]:checked').value;
            update(artwork);
        });
    }
    ...
    frameStyleRadioButtons.forEach(item => {
        item.addEventListener("change", () => {
            artwork.frameStyle = document.querySelector('input[name="frameStyle"]:checked').value;
            update(artwork);
        })
    });
    ...
    matWidthTextbox.addEventListener("change", () => {
        matWidthTextbox.value = clamp(matWidthTextbox.value, 0, 10);
        matWidthTextbox.value = roundTo1Decimal(matWidthTextbox.value);

        matWidthRange.value = matWidthTextbox.value;
        artwork.matWidth = matWidthTextbox.value * 10;
    ...
    matColorRadioButtons.forEach(item => {
        item.addEventListener("change", () => {
            artwork.matColor = document.querySelector('input[name="matColor"]:checked').value;
            update(artwork);
        })
    });
...
function setTotalSizes(img, artwork) {
    ...
    switch (artwork.printSize) {
        case "S":
            printSize = allPrintSizes.S;
            break;
        case "M":
            printSize = allPrintSizes.M;
            break;
        case "L":
            printSize = allPrintSizes.L;
            break;
    }
