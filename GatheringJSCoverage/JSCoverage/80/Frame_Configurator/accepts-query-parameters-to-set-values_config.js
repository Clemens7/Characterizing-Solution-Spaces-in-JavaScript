...
function presetValues(urlParams) {
    if (urlParams.get('printSize')) {
        document.querySelector(`input[name="printSize"][value="${urlParams.get('printSize')}"]`).setAttribute('checked', true);
    }
    if (urlParams.get('frameStyle')) {
        document.querySelector(`input[name="frameStyle"][value="${urlParams.get('frameStyle')}"]`).setAttribute('checked', true);
    }
    if (urlParams.get('frameWidth')) {
        document.querySelector('input[name="frameWidth"]').setAttribute('value', roundAndBoundValue(parseInt(urlParams.get('frameWidth')) / 10, [2, 5]));
        document.querySelector('input[name="frameWidthR"]').setAttribute('value', roundAndBoundValue(parseInt(urlParams.get('frameWidth')) / 10, [2, 5]));
    }
    if (urlParams.get('matColor')) {
        document.querySelector(`input[name="matColor"][value="${urlParams.get('matColor')}"]`).setAttribute('checked', true);
    }
    if (urlParams.get('matWidth')) {
        document.querySelector('input[name="matWidth"]').setAttribute('value', roundAndBoundValue(parseInt(urlParams.get('matWidth')) / 10, [0, 10]));
        document.querySelector('input[name="matWidthR"]').setAttribute('value', roundAndBoundValue(parseInt(urlParams.get('matWidth')) / 10, [0, 10]));
    }
}
