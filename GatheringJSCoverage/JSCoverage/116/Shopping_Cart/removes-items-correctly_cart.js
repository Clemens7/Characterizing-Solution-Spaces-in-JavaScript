...
function label(artwork, elem) {
  ...
  div.childNodes[5].addEventListener('click', () => { removeArtwork(artwork, elem) });
  return div;
}
/**
 * @param artwork {ArtworkFull}
 ...
 **/
function removeArtwork(artwork, elem) {
  //remove from cart
  if (localStorage.getItem('cart') !== null) {
    // The updated cart
    let cart = JSON.parse(localStorage.getItem('cart')).filter((elem) => {
      return elem.objectID !== artwork.oid;
    });
    ...
    //remove from header count
    setHeader(cart.length);

    //remove from display
    if (elem) {
      cart = document.getElementById('cart');
      cart.removeChild(elem);
    }
    //remove from total price
    updatePrice(-1 * artwork.price);
  }
}
/**
