    ...
    function loadImageByObjectID() {
        ...
        if (objectID != null) {
            ...
            if (!isNaN(frameWidth) && frameWidth >= 20 && frameWidth <= 50) {
                frameWidth /= 10;
                document.getElementsByName("frameWidth")[0].value = frameWidth;
                document.getElementsByName("frameWidth")[0].setAttribute("value", frameWidth);
                document.getElementsByName("frameWidthR")[0].value = frameWidth;
                document.getElementsByName("frameWidthR")[0].setAttribute("value", frameWidth);
            }
            let matWidth = parseInt(urlParams.get('matWidth'), 10);
            if (!isNaN(matWidth) && matWidth >= 0 && matWidth <= 100) {
                matWidth /= 10;
                document.getElementsByName("matWidth")[0].value = matWidth;
                document.getElementsByName("matWidth")[0].setAttribute("value", matWidth);
                document.getElementsByName("matWidthR")[0].value = matWidth;
                document.getElementsByName("matWidthR")[0].setAttribute("value", matWidth);
            }

            const printSize = urlParams.get('printSize');
            if (["S", "M", "L"].includes(printSize)) {
                document.querySelector(`input[name="printSize"][value=${printSize}]`).click();
            }
            const frameStyle = urlParams.get('frameStyle');
            if (["classic", "natural", "shabby", "elegant"].includes(frameStyle)) {
                document.querySelector(`input[name="frameStyle"][value=${frameStyle}]`).click();
            }
            const matColor = urlParams.get('matColor');
            if (["ivory", "mint", "wine", "indigo", "coal"].includes(matColor)) {
                document.querySelector(`input[name="matColor"][value=${matColor}]`).click();
            }
