        ...
        function getDescription(item) {
            let description = item.printSize === "L" ? "Large" : (item.printSize === "M" ? "Medium" : "Small");
        ...
        async function createCartItem(item, price) {
            ...
            newItem.getElementsByTagName("button")[0].addEventListener("click", () => remove(item));
        ...
        function remove(item) {
            ...
            cart = cart.filter(element => {
                for (const argumentsKey in element) {
                    if (item[argumentsKey] !== element[argumentsKey]){
                        return true;
                    }
                } return false;});
