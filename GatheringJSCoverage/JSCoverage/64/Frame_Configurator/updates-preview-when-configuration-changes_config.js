...
function connectMatWidthInputs() {
    ...
    const setValue = (value, input, originInput) => {
        value = Math.round(value * 10) / 10;
        input.value = Math.min(10, Math.max(0, value));
        originInput.value = input.value;
        updateChangeableData();
    };

    matWidthInput.addEventListener('change', e => setValue(e.target.value, matWidthSlider, e.target));
...
function attachRangeListeners() {
    ...
    rangeElementNames
        ...
        .forEach(element => {
            element.addEventListener('input', () => updateChangeableData());
        });
}
