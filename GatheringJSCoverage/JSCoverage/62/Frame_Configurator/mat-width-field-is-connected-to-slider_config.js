...
async function loadConfig() {
    ...
    if (objectID) {
        ...
        matInputField.addEventListener("change", function () {
            let help=+Number.parseFloat(matInputField.value).toFixed(1);
            if (help < matSlider.min) 
            if (help > matSlider.max) 
            matSlider.value = help;
            matInputField.value=help;
            render();
        })
