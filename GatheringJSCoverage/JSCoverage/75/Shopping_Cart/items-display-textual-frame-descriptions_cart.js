...
function getDescription(artwork) {
    let description;
    description =
        (artwork.printSize) === 'L' ? 'Large' :
            (artwork.printSize) === 'S' ? 'Small' ;
    if (artwork.matWidth == "0")
        description += " print in a " + artwork.frameWidth/10 + " cm "
            + artwork.frameStyle + " frame.";
    else {
...
async function retrieveArtwork(objectId) {
    if (localStorage.getItem("artObj" + objectId) != null)  else {
        const response = await fetch("https://collectionapi.metmuseum.org/public/collection/v1/objects/" + objectId);
        const rawResponse = await response.json();


        const artworkData = {
            'objectID': rawResponse.objectID,
            'title': rawResponse.title,
            'artistName': rawResponse.artistDisplayName,
            'objectDate': rawResponse.objectDate,
            'img': rawResponse.primaryImage,
            'primaryImageSmall': rawResponse.primaryImageSmall
        };


        localStorage.setItem("artObj" + objectId, JSON.stringify(artworkData));
        return artworkData;
    }}
