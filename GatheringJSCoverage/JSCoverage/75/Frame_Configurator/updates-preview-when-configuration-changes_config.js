...
frameField.addEventListener('input', function (e) {
    if (e.target.value !== "") {
        frameRange.value = e.target.value;
        frameField.value = e.target.value;
        updatePreview()
    }
});

function functionFrameFieldLostFocus() { // hack due to test doing wierd stuff
    frameField.value = roundFrame(frameField.value);
}
...
function roundFrame(numbertoround) {
...
function calculateTotalSize(img, printSize, frameWidth, matWidth) {
    ...
    if (printSize === "S") {
        chosenSizeX = getPrintSizes(img).S[0];
        chosenSizeY = getPrintSizes(img).S[1];
    }
    ...
    if (printSize === "L") {
        chosenSizeX = getPrintSizes(img).L[0];
        chosenSizeY = getPrintSizes(img).L[1];
    }
