...
export function removeFromCart(entryUUID) {
    const cartContent = getCartContent();
    const cartEntry = cartContent.find(entry => entry.entryUUID === entryUUID);
    if (cartEntry) {
        cartContent.splice(cartContent.indexOf(cartEntry), 1);
        localStorage.setItem(LS_CART_KEY, JSON.stringify(cartContent));
    }
}
