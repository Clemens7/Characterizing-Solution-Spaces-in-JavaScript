...
function updateCart() {
    ...
    if (cart) {
        ...
        for (const item of cartItems) {
            if (!cartContent.find(value => value.entryUUID === item.getAttribute('id'))) {
                toRemove.push(item);
            }
        }
        ...
        for (const item of cartContent) {
            ...
            subTotal += price;
            if (!cartItems.find(value => value.getAttribute('id') === item.entryUUID)) {
...
function addCartItem(cart, price, art, item) {
    ...
    document.getElementById(`remove-${item.entryUUID}`).addEventListener('click', () => {
        CartCache.removeFromCart(item.entryUUID);
        updatePage();
    });
