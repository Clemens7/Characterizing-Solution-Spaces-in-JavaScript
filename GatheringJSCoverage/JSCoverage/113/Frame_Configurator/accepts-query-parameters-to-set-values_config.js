...
const presetConfiguration = async () => {
    ...
    if (printSize) {
        form.elements['printSize'].value = printSize;
    }
    let frameStyle = getQueryParameter('frameStyle');
    if (frameStyle) {
        form.elements['frameStyle'].value = frameStyle;
    }
    let frameWidth = getQueryParameter('frameWidth');
    console.log(frameWidth);
    if (frameWidth) {
        console.log(frameWidth.toString());
        form.elements['frameWidth'].value = frameWidth / 10;
        document.getElementById('frameSlider').value = frameWidth / 10;
    }
    let matColor = getQueryParameter('matColor');
    if (matColor) {
        form.elements['matColor'].value = matColor;
    }
    let matWidth = getQueryParameter('matWidth');
    if (matWidth) {
        form.elements['matWidth'].value = matWidth / 10;
