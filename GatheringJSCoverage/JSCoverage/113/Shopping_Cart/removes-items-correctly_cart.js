...
const museumLabel = (data, item, idx) => {
    ...
    cartRemove.onclick = () => {
        let rmItem = shoppingCart[idx];
        console.log(`remove item ${idx} (objectID ${rmItem.objectID}) from shoppingCart`);

        /* todo: remove nth element from shoppingcart and then localStorage */
        shoppingCart.splice(idx, 1);
        console.log(shoppingCart);
        localStorage.setItem('cart', JSON.stringify(shoppingCart));
        // should be unnecessary, as it gets reset in cartItems(shoppingCart)
        // priceList.splice(idx, 1);

        /* reconstruct the site with updated shoppingcart */
        cartItems(shoppingCart);
    };
