...
export function initConfigurator() {
    ...
    const frameWidth = (urlParams.get('frameWidth') ? Math.min(Math.max(urlParams.get('frameWidth'), 20), 50) ) / 10;
    form['frameWidth'].value = frameWidth;
    form['frameWidthR'].value = frameWidth;

    const matWidth = (urlParams.get('matWidth') ? Math.min(Math.max(urlParams.get('matWidth'), 0), 100) ) / 10;
