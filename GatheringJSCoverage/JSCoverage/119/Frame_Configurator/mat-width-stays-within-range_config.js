...
export function updateFrame() {
    ...
    render(image, container, printSize.value, frameStyle.value, frameWidth.value * 10, matColor.value, matWidth.value * 10);
    totalPrice(printSize.value, frameStyle.value, frameWidth.value, matWidth.value);
    totalSize(printSize.value, image, frameWidth.value, matWidth.value);
...
function totalSize(printSize, image, frameWidth, matWidth) {
    console.log(getPrintSizes(image));
    var dict = getPrintSizes(image);
    ...
    var currentSize = dict[printSize];
    console.log(currentSize);
    var totalSizeW = currentSize[0];
    var totalSizeH = currentSize[1];
    console.log(frameWidth);

    const totalSize = document.getElementById("total-size");

    var totalSizeWRounded = Math.round(((totalSizeW / 10) + (frameWidth * 2) + (matWidth * 2)) * 10) / 10;
    var totalSizeHRounded = Math.round(((totalSizeH / 10) + (frameWidth * 2) + (matWidth * 2)) * 10) / 10;

    totalSize.innerHTML =
        `
        ${totalSizeWRounded} x ${totalSizeHRounded} cm
        `
}