...
if (params.get("frameWidth")) {
    form["frameWidth"].value = form["frameWidthR"].value = Math.round(Math.max(Math.min(params.get("frameWidth") / 10, 5), 2) * 10) / 10;
}
if (params.get("matColor")) {
    form["matColor"].value = params.get("matColor");
}
if (params.get("matWidth")) {
    form["matWidth"].value = form["matWidthR"].value = Math.round(Math.max(Math.min(params.get("matWidth") / 10, 10), 0) * 10) / 10;
}
