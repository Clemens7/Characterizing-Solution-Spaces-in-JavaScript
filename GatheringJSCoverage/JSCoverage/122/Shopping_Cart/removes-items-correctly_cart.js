...
async function createCartItemNode(item) {
    ...
    removeButtonNode.addEventListener('click', event => newRemoveCartItem(removeButtonNode, item['objectID']));
...
async function newRemoveCartItem(button = null, id = null) {
    if (!button || !id) 
    let cart = await getCart();
    storeCart(cart.filter(item => item['objectID'] !== id));

    let cartItem = button.parentNode.parentNode;
