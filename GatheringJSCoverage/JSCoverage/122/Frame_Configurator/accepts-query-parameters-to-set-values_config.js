...
async function retrieveImgData(imgID, params) {
  ...
  //update config
  if (params.has('printSize') && params.has('frameStyle') &&
      params.has('frameWidth') && params.has('matColor') && params.has('matWidth')) {
    updatePage(params);
  }
...
async function updatePage(params) {
  ...
  selectRadioButton('print-size-', printSize, ['s', 'm', 'l']);

  const frameStyle = params.get('frameStyle');
  selectRadioButton('frame-style-', frameStyle, ['classic', 'natural', 'shabby', 'elegant']);

  const frameWidth = params.get('frameWidth');
  setNumberValue('frameWidth', frameWidth);
  setNumberValue('frameWidthR', frameWidth);

  const matColor = params.get('matColor');
  selectRadioButton('mat-color-', matColor, ['ivory', 'mint', 'wine', 'indigo', 'coal']);

  const matWidth = params.get('matWidth');
  setNumberValue('matWidth', matWidth);
  setNumberValue('matWidthR', matWidth);
...
function setNumberValue(name, value) {
  ...
  document.getElementsByName(name)[0].value = valueInCm;
}

function selectRadioButton(defaultId, value, options) {
  let selectedId = defaultId + value.trim().toLowerCase();
  let i;
