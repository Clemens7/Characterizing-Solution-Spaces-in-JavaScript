    ...
    function setConfigurationListeners() {
      for (const printSizeTag of document.getElementsByName("printSize")) {
        printSizeTag.addEventListener("change", () => { setPrice(); updateConfigurator(); });
      }
      for (const frameStyleTag of document.getElementsByName("frameStyle")) {
        frameStyleTag.addEventListener("change", () => { setPrice(); updateConfigurator(); });
      }
      for (const matColorTag of document.getElementsByName("matColor")) {
        matColorTag.addEventListener("change", updateConfigurator)
      }
      document.getElementsByName("frameWidth")[0].addEventListener("change", () => { setPrice(); updateConfigurator(); });
      document.getElementsByName("matWidth")[0].addEventListener("change", () => { setPrice(); updateConfigurator(); });
    ...
    function setWidthListener(original, copy, correctionFunction) {
      for (const originalElement of document.getElementsByName(original)) {
        for (const copyElement of document.getElementsByName(copy)) {
          originalElement.addEventListener("change", function () {
            originalElement.value = correctionFunction(originalElement.value * 10) / 10;
            if (originalElement.value !== copyElement.value) {
