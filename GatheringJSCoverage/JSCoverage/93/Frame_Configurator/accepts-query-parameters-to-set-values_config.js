...
RenderedImage.prototype.updateControls = async function(){
    ...
    document.getElementById("frameWidth").setAttribute('value',
        this.urlParams.get("frameWidth") ? (this.frameWidth = this.urlParams.get("frameWidth"))/10 );
    document.getElementById("frameWidthR").setAttribute('value',
        this.urlParams.get("frameWidth") ? (this.frameWidth = this.urlParams.get("frameWidth"))/10 );
    document.getElementById("matWidthR").setAttribute('value',
        this.urlParams.get("matWidth") ? (this.matWidth = this.urlParams.get("matWidth"))/10 );
    document.getElementById("matWidth").setAttribute('value',
        this.urlParams.get("matWidth") ? (this.matWidth = this.urlParams.get("matWidth"))/10 );
    document.getElementById(`print-size-${this.urlParams.get("printSize") ? (this.printSize = this.urlParams.get("printSize")).toLowerCase() 
        }`).checked = true;
    document.getElementById(`frame-style-${this.urlParams.get("frameStyle") ? this.frameStyle = this.urlParams.get("frameStyle") }`).checked = true;
    document.getElementById(`mat-color-${this.urlParams.get("matColor") ? this.matColor = this.urlParams.get("matColor") }`).checked = true;
};
