...
RenderedImage.prototype.invalidReset = function (form, input, range, min, max) {
    if (!form.checkValidity()) {
        if (parseFloat(input.value) > parseFloat(max)){
            input.value = range.value = max;
        }else if (parseFloat(input.value) < parseFloat(min)){
            input.value = range.value = min;
        }else {
            range.value = input.value = Math.round(parseFloat(input.value)*10)/10;
        }
        return true;
    }};
