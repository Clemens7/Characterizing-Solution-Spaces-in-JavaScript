    ...
    document.addEventListener('DOMContentLoaded', event => {
        ...
        if (printSize) { setRadioButton("printSize", printSize); }
        if (frameStyle) { setRadioButton("frameStyle", frameStyle); }
        if (frameWidth) { setFrameWidth(frameWidth); }
        if (matColor) { setRadioButton("matColor", matColor); }
        if (matWidth) { setMatWidth(matWidth); }
    ...
    function setRadioButton(name, value) {
      let btnList = document.getElementsByName(name);
    ...
    function setFrameWidth(value) {
      ...
      let frameSlider = document.getElementById("frameWidthSlider");
      if (num >= 20 && num <= 50) {
        frameTxt.value = num/10;
        frameSlider.value = num/10;
      }
    }
