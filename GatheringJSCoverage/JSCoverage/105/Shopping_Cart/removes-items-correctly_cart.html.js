    ...
    function remove_cart_item(id, price){
      let my_cart = JSON.parse(localStorage.cart);
      console.log(my_cart);
      for (let i = 0; i <= my_cart.length; i++) {
        console.log(my_cart[i]);
        console.log(id);
        console.log(price);
        if (my_cart[i].objectID === id){
          //update total price
          document.getElementById("price-total").innerText = ''+((parseFloat(document.getElementById("price-total").innerText) - price).toFixed(2));
          let dom_elem = document.getElementById("preview-container-"+id).parentElement;
    ...
    async function fill_cart_item_with_content(cart_item, objectID, price_value, description, href, c) {
      ...
      cart_item.querySelector(".cart-remove").onclick = function() { remove_cart_item(objectID, price_value)};
