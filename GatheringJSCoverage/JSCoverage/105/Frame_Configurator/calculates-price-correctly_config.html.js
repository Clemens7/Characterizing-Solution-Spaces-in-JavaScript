  ...
  frameWidthN.addEventListener("change",function(){
    frameWidth = minMaxRound(2,5,frameWidthN.value);
    frameWidthN.value = frameWidth;
    frameWidthR.value = frameWidth;
    updateFrame();
  });
  ...
  allPrintSizes[0].addEventListener('change', function(){
    printSize = allPrintSizes[0].value;
    //localStorage.clear();
    updateFrame();
  });
  allPrintSizes[1].addEventListener('change', function(){
    printSize = allPrintSizes[1].value;
    updateFrame();
  });
  allPrintSizes[2].addEventListener('change', function(){
    printSize = allPrintSizes[2].value;
    updateFrame();
  });

  const allFrameStyles = document.getElementsByName("frameStyle");
  allFrameStyles[0].addEventListener('change', function(){
    frameStyle = allFrameStyles[0].value;
    updateFrame();
  });
  allFrameStyles[1].addEventListener('change', function(){
    frameStyle = allFrameStyles[1].value;
    updateFrame();
  });
  allFrameStyles[2].addEventListener('change', function(){
    frameStyle = allFrameStyles[2].value;
    updateFrame();
  });
  allFrameStyles[3].addEventListener('change', function(){
    frameStyle = allFrameStyles[3].value;
    updateFrame();
  });

  const allMatColors = document.getElementsByName("matColor");
  allMatColors[0].addEventListener('change', function(){
    matColor = allMatColors[0].value;
    updateFrame();
  });
  allMatColors[1].addEventListener('change', );
  allMatColors[2].addEventListener('change', function(){
    matColor = allMatColors[2].value;
    updateFrame();
  });
  allMatColors[3].addEventListener('change', );
  allMatColors[4].addEventListener('change', function(){
    matColor = allMatColors[4].value;
    updateFrame();
  });
