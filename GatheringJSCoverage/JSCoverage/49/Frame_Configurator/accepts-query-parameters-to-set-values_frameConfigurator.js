...
const isValidOption = {
    printSize: s => ['S', 'M', 'L'].includes(s),
    frameStyle: s => ['classic', 'elegant', 'natural', 'shabby'].includes(s),
    frameWidth: w => w >= 2 && w <= 5 && Math.floor(w * 10) / 10 === w,
    matColor: c => ['ivory', 'mint', 'wine', 'indigo', 'coal'].includes(c),
...
function setRadioOption(name, value) {
    if (isValidOption[name](value)) {
        $all(`input[type='radio'][name='${name}']`).forEach(n => n.removeAttribute('checked'));
...
export function setOption(option, value) {
    ...
    if (radioOptions.includes(option)) {
        setRadioOption(option, value);
    } else if (rangeOptions.includes(option)) {
...
export function setQueryConfigOptions() {
    for (let option of ['printSize', 'frameStyle', 'frameWidth', 'matColor', 'matWidth']) {
        ...
        if (value) {
            if (['frameWidth', 'matWidth'].includes(option)) {
                value /= 10;
            }
