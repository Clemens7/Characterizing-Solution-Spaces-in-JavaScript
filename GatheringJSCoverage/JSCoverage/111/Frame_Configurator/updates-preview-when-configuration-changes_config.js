...
matWidthInput.addEventListener("keypress", function(event) {
    if (event.key === 'Enter') 
});

matWidthInput.addEventListener("focusout", function(event) {
    calculateValue(matWidthInput, 4);
    updateMatRangeSlider(matWidthInput, 4);
    renderPreviewImage();
});
