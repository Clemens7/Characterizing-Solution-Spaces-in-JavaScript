...
select.addEventListener('change', event => {
  let shippingCosts = destinations[event.target.selectedIndex].cost;
  document.getElementById('price-shipping').innerText = `${(shippingCosts / 100).toFixed(2)}`;
  document.getElementById('price-total').innerText = `${(shippingCosts / 100 + subtotal)}`;
});
