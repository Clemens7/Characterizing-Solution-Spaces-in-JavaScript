...
for (const [key, value] of params) {
    if (key === 'objectID') {
        ...
        config.printSize = value.toUpperCase();

        const sizeRadioButton = document.querySelector(`input[value = ${config.printSize}]`);
        sizeRadioButton.checked = true;
    } else if (key === 'frameStyle') {
        config.frameStyle = value.toLowerCase()

        const frameStyleRadioButton = document.querySelector(`input[value = ${config.frameStyle}]`);
        frameStyleRadioButton.checked = true;
    } else if (key === 'frameWidth') {
        config.frameWidth = sanitize(value/10, 2, 5);

        frameWidthField.value = config.frameWidth;
        frameWidthSlider.value = config.frameWidth;
    } else if (key === 'matColor') {
        config.matColor = value.toLowerCase();

        const matColorRadioButton = document.querySelector(`input[value = ${config.matColor}]`);
        matColorRadioButton.checked = true;
    } else if (key === 'matWidth') {
        config.matWidth = sanitize(value/10, 0, 10);

        matWidthField.value = config.matWidth;
        matWidthSlider.value = config.matWidth;
    }
}
...
function sanitize(input, lower, upper) {
    if (input > upper)  else if (input < lower) 
    return (Math.round((parseFloat(input) + Number.EPSILON) * 10) / 10);
}
