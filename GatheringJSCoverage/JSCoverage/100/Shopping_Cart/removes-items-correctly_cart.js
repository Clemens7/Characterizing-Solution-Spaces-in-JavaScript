...
async function loadDataAndPopulateUI() {
    ...
    try {
        ...
        for (let e of elements) {
            ...
            }catch { // otherwise load and fetch metadata from API
                await fetchJsonFromAPI(e.objectID)
                    .then(response => {
                        populateItems(response, e, elements)
                    })
            }
        }
    } 
...
function createCartElement(id, item, artworkData) {
    ...
    removeButton.addEventListener('click', _ => { removeFromCart(id); });
...
function removeFromCart(id) {
    updateTotalPrice(id);
    const kids = [].slice.call(cart.children);  // convert HTMLCollection to Array
    let count = 0;
    for (let kid of kids) {
        if (kid.id === `cart-item-${id}`) {
            cart.removeChild(kid); break; }
        else count++
    }
    let jsonItems = localStorage.getItem('cart');
    const elements = JSON.parse(jsonItems);
    elements.splice(count, 1);
...
function updateTotalPrice(id) {
    const decPrice = parseFloat(document.getElementById(`price-${id}`).innerText);
    priceTotal -= decPrice;
    const totalPriceLabel = document.getElementById("price-total");
    totalPriceLabel.innerText = priceTotal;
}
