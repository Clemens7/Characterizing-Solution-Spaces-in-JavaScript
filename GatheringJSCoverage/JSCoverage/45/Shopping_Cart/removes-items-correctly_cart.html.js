    ...
    function itemToHtml(item, ur, ar, ti, da, cont){
      ...
      button.addEventListener("click", function(){removeCartItem(item.objectID)});
        ...
        if (localStorage[key] === "" || localStorage[key] === "[]")  else {
          ...
          for(let item of revObjects){
            if (item.objectID in localStorage){
              // If this objectID is present in the cache of the local storage, we parse the JSON file and display data
    ...
    function removeCartItem(objectID) {
      ...
      if (key in localStorage) {
        ...
        localStorage[key] = JSON.stringify(objects);
        console.log("Removed item with ID: " + objectID);
        let container = document.getElementById('cart');
        let cartTotal = container.lastChild;
        container.innerHTML = '';
        container.append(cartTotal);
        total = 0;
        showArtWorks(container);
      }
    }
