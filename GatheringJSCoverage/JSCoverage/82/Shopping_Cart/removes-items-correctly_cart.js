...
function generateCart(cartWorking){
	...
	for (let i = 0; i < getCartLength(); i++) {
        ...
        document.querySelector(`#remove-${objectID}-${occurrence}`).addEventListener('click', function(){removeFromCart(i, occurrence)});
 ...
 * */
function removeFromCart(id, occurrence){
	let storageCart = getStorageCart();
	storageCart.splice(id,1);
