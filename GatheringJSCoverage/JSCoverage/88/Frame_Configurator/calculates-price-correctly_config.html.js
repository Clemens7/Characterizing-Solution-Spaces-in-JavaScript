        ...
        .then(async function (data) {
            if (data.status !== 200)  else {
                ...
                img.addEventListener('load', function () {
                    ...
                    document.getElementById('print-size-s').addEventListener('click', function () {
                    ...
                    document.getElementById('print-size-m').addEventListener('click', function () {
                        printSize = this.value;
                        render(img, imgContainer, printSize, frameStyle, frameWidth * 10, matColor, matWidth * 10);
                        let price = calculatePrice(printSize, frameStyle, frameWidth, matWidth);
                        if (!Number.isInteger(price) && Number.isInteger(price * 10)) 
                    ...
                    document.getElementById('print-size-l').addEventListener('click', function () {
                        printSize = this.value;
                        render(img, imgContainer, printSize, frameStyle, frameWidth * 10, matColor, matWidth * 10);
                        let price = calculatePrice(printSize, frameStyle, frameWidth, matWidth);
                        if (!Number.isInteger(price) && Number.isInteger(price * 10)) {
                    ...
                    document.querySelectorAll('.frame-style-item').forEach(elem => {
                        elem.addEventListener('change', function () {
                            frameStyle = this.querySelector("input[name='frameStyle']").getAttribute('value');
                            render(img, imgContainer, printSize, frameStyle, frameWidth * 10, matColor, matWidth * 10);
                            let price = calculatePrice(printSize, frameStyle, frameWidth, matWidth);
                            if (!Number.isInteger(price) && Number.isInteger(price * 10)) {
                                price = price.toString() + '0';
                    ...
                    document.querySelectorAll('.mat-color-item').forEach(elem => {
                        elem.addEventListener('change', function () {
                            matColor = this.querySelector("input[name='matColor']").getAttribute('value');
                            render(img, imgContainer, printSize, frameStyle, frameWidth * 10, matColor, matWidth * 10);
                        })
                    });
