...
async function addCartItem(item, index) {
    ...
    newItem.querySelector(".cart-remove").addEventListener('click', function removeItem(e) {
        let item = document.getElementById(`cart-index-${e.target.id}`);
        item.remove();
        cartItems.deleteItem(e.target.id)
...
function updateItemIndex() {
    let itemDivs = document.getElementsByClassName('cart-item');
    let length = itemDivs.length - 1;
    for (let i = 0; i < itemDivs.length; i++) {
        let item = itemDivs[i];
        item.id = 'cart-index-' + length;
        let button = item.getElementsByClassName('cart-remove');
        button[0].id = length;
        length--;
    }
    updateTotalPrice();
...
function updateTotalPrice() {
    let priceItems = document.getElementsByClassName('piece-price');
    let totalPrice = 0;
    for (let i = 0; i < priceItems.length; i++) {
        totalPrice += parseFloat(priceItems[i].innerHTML);
    }
    document.querySelector('#price-total').textContent = totalPrice.toFixed(2);
}
