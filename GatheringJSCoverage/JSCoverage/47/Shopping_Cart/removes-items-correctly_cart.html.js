        ...
        function removeItem(item, object) {
            var modifiedCart = Array();
            let cart = JSON.parse(localStorage.getItem('cart'));
            for (let o of cart) {

                if (JSON.stringify(o) !== JSON.stringify(object)) {
                    modifiedCart.push(o);
                }
            }
            cart = modifiedCart;
            sumTotal -= calculatePrice(object.printSize, object.frameStyle, object.frameWidth, object.matWidth);
            total.textContent = sumTotal.toFixed(2) + "";
            localStorage.setItem('cart', JSON.stringify(cart));
