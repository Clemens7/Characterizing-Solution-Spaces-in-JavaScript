      ...
      matColors.forEach(matColor=>{matColor.addEventListener('change', function(){
          updateImage();
      })});

      frameStyles.forEach(frameStyle=>{frameStyle.addEventListener('change', function(){
          updateImage();
      })});

      printSizes.addEventListener('change', function(){
          updateImage();
      });
      ...
      matWidthInput.addEventListener('change', function(){
          if(matWidthInput.value<0)
              
          if(matWidthInput.value>10)
              
          matWidthSlider.value = parseFloat(parseFloat(matWidthInput.value).toFixed(1));
          matWidthInput.value = parseFloat(parseFloat(matWidthInput.value).toFixed(1));
          updateImage();
      });
