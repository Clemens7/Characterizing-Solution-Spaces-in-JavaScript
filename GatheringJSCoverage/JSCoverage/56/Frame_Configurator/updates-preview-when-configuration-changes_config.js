...
export function setFrameWidth(value){
    value = Math.min(value, 5);
    value = Math.max(value, 2);
    value = Math.round(value * 10) / 10;
    const numberElem = document.getElementsByName('frameWidth');
    const rangeElem = document.getElementsByName('frameWidthR');
...
export function setRadioParameter(parameter, value) {
    if (value) {  // value not null
        const elements = document.getElementsByName(parameter);
