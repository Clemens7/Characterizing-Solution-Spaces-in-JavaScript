    ...
    function configureFrameConfig(urlParams) {
      if (urlParams.has('printSize')) {
        ...
        let printSizeFieldSet = document.getElementById("print-size-fieldset");
        let inputs = printSizeFieldSet.getElementsByTagName('input');
        for (let i = 0; i < inputs.length; ++i) {
          if (inputs[i].value === printSize) {
      ...
      if (urlParams.has('frameWidth')) {
        const frameWidth = parseInt(urlParams.get('frameWidth')) / 10;
        updateFrameWidth(frameWidth);
      }

      if (urlParams.has('frameStyle')) {
        const frameStyle = urlParams.get('frameStyle');
        let frameStyleFieldSet = document.getElementById("frame-style-fieldset");
        let inputs = frameStyleFieldSet.getElementsByTagName('input');
        for (let i = 0; i < inputs.length; ++i) {
          if (inputs[i].value === frameStyle) {
            inputs[i].checked = true;
          }
        }
      }
      ...
      if (urlParams.has('matWidth')) {
        const matWidth = parseInt(urlParams.get('matWidth')) / 10;
        updateMatWidth(matWidth);
      }

      if (urlParams.has('matColor')) {
        const matColor = urlParams.get('matColor');
        let matColorFieldSet = document.getElementById("mat-color-fieldset");
        let inputs = matColorFieldSet.getElementsByTagName('input');
        for (let i = 0; i < inputs.length; ++i) {
          if (inputs[i].value === matColor) {
            inputs[i].checked = true;
          }
        }
      }
