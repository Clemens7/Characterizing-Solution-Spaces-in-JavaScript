...
function frameInputToSlider() {
    const frameWidth = document.getElementById('frame-width');
    const slider = document.getElementById('frameWidthSlider');
    slider.value = frameWidth.value;
}



function matInputToSlider() {
    const matWidth = document.getElementById('mat-width');
    const slider = document.getElementById('matWidthSlider');
    slider.value = matWidth.value;
}

function validateFrameWidth() {
    const element = document.getElementById('frame-width');
    let frameWidth = parseFloat(element.value);
    if (frameWidth < 2)  else if (frameWidth > 5)  else {
        frameWidth = Math.round((frameWidth + Number.EPSILON) * 10) / 10;
    }
    element.value = frameWidth;
}

function validateMatWidth() {
    const element = document.getElementById('mat-width');
    let matWidth = parseFloat(element.value);
    if (matWidth < 0)  else if (matWidth > 10)  else {
        matWidth = Math.round((matWidth + Number.EPSILON) * 10) / 10;
    }
    element.value = matWidth;
}
