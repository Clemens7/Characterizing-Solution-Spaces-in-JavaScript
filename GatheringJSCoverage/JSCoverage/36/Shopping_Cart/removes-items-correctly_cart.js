...
function createButton(artwork, cartItem) {
    ...
    button.addEventListener('click', event => {
        ...
        for (let index = 0; index < cart.length; index++) {
            if (JSON.stringify(cartItem) === JSON.stringify(cart[index])) {
                // removes item from localStorage
                cart.splice(index, 1);
                localStorage["cart"] = JSON.stringify(cart);
                // removes item from DOM
                let elemToRemove = document.getElementById(`preview-container-${hash(cartItem)}`).parentNode;
                elemToRemove.parentNode.removeChild(elemToRemove);
                // adjust the rest of the page
                calculateTotalPrice();
                if (cart.length === 0) 
                document.getElementById('cart-link').innerText = cart.length === 0  : `Cart (${cart.length})`;
            }
