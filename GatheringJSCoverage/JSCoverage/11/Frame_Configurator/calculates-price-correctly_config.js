    ...
    updateInput() {
        ...
        if (this.pSize !== null) {
            switch (this.pSize.toUpperCase()) {
                case 'S':
                    printSize[0].checked = true;
                    printSize[1].checked = false;
        ...
        if (this.fStyle !== null) {
            switch (this.fStyle.toLowerCase()) {
                case "classic":
                    frameStyle[0].checked = true;
                    frameStyle[1].checked = false;
        ...
        if (this.mColor !== null){
            switch (this.mColor.toUpperCase()) {
                case 'IVORY':
                    matColor[0].checked = true;
                    matColor[1].checked = false;
...
function addInputListeners(){
    frameWidthText.addEventListener("change", val => changeFrameWidth(val.target.value));
    ...
    for(let i = 0, max = printSize.length; i < max; i++) {
        printSize[i].addEventListener("click", val => changePrintSize(val.target.value));
    }
    for(let i = 0, max = frameStyle.length; i < max; i++) {
        frameStyle[i].addEventListener("click", val => changeFrameStyle(val.target.value));
    }
    for(let i = 0, max = matColor.length; i < max; i++) {
        matColor[i].addEventListener("click", val => changeMatColor(val.target.value));
    }
}
...
function changeFrameWidth(val) {
    if (val >= 2 && val <= 5) {
        currentArtwork.fWidth = Math.round(val * 10) / 10;
        currentArtwork.updateInput();
    }
}
...
function changePrintSize(XYZ) {
    currentArtwork.pSize = XYZ;
    currentArtwork.updateInput();
}

function changeFrameStyle(style) {
    currentArtwork.fStyle = style;
    currentArtwork.updateInput();
}

function changeMatColor(color) {
    currentArtwork.mColor = color;
    currentArtwork.updateInput();
}