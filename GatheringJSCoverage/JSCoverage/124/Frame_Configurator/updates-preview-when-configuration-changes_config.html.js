    ...
    els.radio.forEach(element => {
      element.addEventListener('click', event => {
        allConfig.config[event.target.name] = event.target.value;
        renderConfig();
      })
    });
    ...
    els.fWidthF.addEventListener('change', function (e) {
      let val = e.target.value;
      allConfig.config.frameWidth = val > 5  : val < 2  : (Math.round((parseFloat(val) + Number.EPSILON) * 10) / 10);;
