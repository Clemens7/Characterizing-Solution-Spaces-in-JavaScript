...
function updateCart(event) {
    var cartItemIndex = Number(event.target.parentElement.childNodes[1].lastChild.id.split("-")[1]);
    cart.remove(cartItemIndex);
    document.getElementById(`preview-container-${cartItemIndex}`).parentElement.remove();
    document.getElementById("price-total").innerText = `${Number(cart.getTotalPrice).toFixed(2)}`;
    for (var index = cartItemIndex + 1; index < document.getElementById("cart").children.length; index++) {
        document.getElementById(`preview-container-${index}`).id = `preview-container-${index - 1}`;
        document.getElementById(`price-${index}`).id = `price-${index - 1}`;
    }
