...
class RangeTextRadioConfig {
    ...
    set width(value) {
        ...
        this.setValueWithinElementBoundaries(this._rangeSlider, value);
        this.setValueWithinElementBoundaries(this._numberInput, value);
    }
    set radioInputs(value) {
        for (let radioInput of this._radioInputs) {
            if (radioInput.id.includes(value)) {
    ...
    setValueWithinElementBoundaries(element, value) {
        let min = Number(element.min);
        let max = Number(element.max);
        let changedValue = Number(value) / 10;
        if (min <= changedValue && changedValue <= max) {
            element.value = changedValue.toString();
        }
    }
    addEventHandler() {
...
export class FrameConfig extends RangeTextRadioConfig {
    ...
    set style(style) {
        if (!(style in FrameStyle))
            
        super.radioInputs = style;
    }
    get selectedStyle() {
...
export class MatConfig extends RangeTextRadioConfig {
    ...
    set color(color) {
        if (!(color in MatColor))
            
        super.radioInputs = color;
    }
    get selectedColor() {
        let selectedInput = super.selectedRadioInput;
        if (selectedInput == null) 
...
export class PrintSizeConfig {
    ...
    set printSize(printSize) {
        if (!(printSize.toUpperCase() in PrintSize))
            
        for (let printSizeInput of this._printSizeInputs) {
            let idToBeSet = "print-size-" + printSize.toLocaleLowerCase();
            if (printSizeInput.id == idToBeSet) {
                printSizeInput.setAttribute("checked", "checked");
            }
            else {
                printSizeInput.removeAttribute("checked");
            }
        }
    }
...
export class QueryParams {
    ...
    setHTMLElements(frameConfig, matConfig, printSizeConfig, addToCartButton) {
        for (let key in this._queryParams) {
            ...
            switch (key) {
                case "frameStyle":
                    frameConfig.style = value;
                    break;
                case "matColor":
                    matConfig.color = value;
                    break;
                case "printSize":
                    printSizeConfig.printSize = value;
                    break;
                case "frameWidth":
                    frameConfig.width = value;
                    break;
                case "matWidth":
                    matConfig.width = value;
                    break;
                case "objectID":
