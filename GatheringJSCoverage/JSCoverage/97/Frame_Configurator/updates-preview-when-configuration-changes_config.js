...
frameWidth.addEventListener("change", event => {
    let clipped = String(Math.round(Math.max(2.0, Math.min(5.0, Number(frameWidth.value))) * 10) / 10);
    frameWidthR.value = clipped;
    let fw = Math.round(Number(frameWidthR.value) * 10) / 10;
    frameWidth.value = String(fw);
    frameWidthValue = fw;
    refresh();
});
