    ...
    function updateShippingCost() {
      let selection = countryDropdown.options[countryDropdown.selectedIndex].text;
      for (let i in destinations.destinations) {
        if (destinations.destinations[i].displayName === selection) {
          shipping.innerText = (destinations.destinations[i].cost / 100).toFixed(2);
          calculateTotal();
        }
      }
    }
