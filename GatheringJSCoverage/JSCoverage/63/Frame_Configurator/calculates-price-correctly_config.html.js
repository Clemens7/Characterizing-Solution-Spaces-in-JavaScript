    ...
    function initForm() {
      ...
      for (const size of form.printSizes) {
        size.addEventListener('change', event => {
          config.printSize = event.target.value;
          updateFrame();
        });
      ...
      for (const frame of form.frameStyles) {
        frame.addEventListener('change', event => {
          config.frameStyle = event.target.value;
          console.log(config.frameStyle);
          updateFrame();
        });
      ...
      for (const color of form.matColors) {
        color.addEventListener('change', event => {
          config.matColor = event.target.value;
          updateFrame();
        });
      ...
      form.matWidth.addEventListener('change', event => {
        config.matWidth = form.matWidth.value * 10;
