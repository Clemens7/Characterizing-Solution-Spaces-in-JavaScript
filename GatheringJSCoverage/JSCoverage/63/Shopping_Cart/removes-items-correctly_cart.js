    ...
    if (cart.length == 0) 
      ...
      for (let item of cart) {
       ...
       // document.getElementById('config-linker').href='./config.html'+`?objectID=${item.objectID}&printSize=${item.printSize}&frameWidth=${item.frameWidth}&frameStyle=${item.frameStyle}&matWidth=${item.matWidth}&matColor=${item.matColor}`;
        ...
        removeButton.onclick = function () { remove(item.objectID, itemPrice) };
...
function remove(objectID, itemPrice) {
  let cart = JSON.parse(localStorage.getItem("cart"));
  //cart.removeItem(objectID);
  cart = cart.filter(function (cartItem) {
    return cartItem.objectID != objectID;
  });
  ...
  const deleteItem=document.getElementById(`cart-item-${objectID}`)
  deleteItem.parentNode.removeChild(deleteItem);

  document.getElementById("price-total").innerText=document.getElementById("price-total").innerText-itemPrice;
  */
}
